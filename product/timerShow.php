...
$strSQL = "SELECT EDate,PID FROM product WHERE TNo = '".mysqli_real_escape_string($con,$_GET['type'])."' ";
...
while($objResult = mysqli_fetch_array($objQuery))
    {
        ...
        echo "<script>setInterval('countDown()',1);</script>";
...
function countDown(){
    ...
    if(timeDifference>=0){
        ...
        if (hour < 0) {
                    showPart=document.getElementById('showRemain' + number); showPart.innerHTML="หมดเวลา"; document.cookie = "end=1";  return;}
        if (hour <= 9) {
